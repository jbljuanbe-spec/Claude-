...
export const LOGROS = [
  { id: 'campeon', emoji: '🏆', nombre: 'Campeón', desc: e => `Levantaste ${e.ligasGanadas} título${e.ligasGanadas > 1 ? 's' : ''} de Liga.`, pista: 'Gana un título de Liga o regional.', cond: e => e.ligasGanadas >= 1 },
  { id: 'mundial', emoji: '🌍', nombre: 'Campeón del Mundo', desc: e => `${e.mundiales} Campeonato${e.mundiales > 1 ? 's' : ''} Mundial${e.mundiales > 1 ? 'es' : ''}. El mejor del planeta.`, pista: 'Gana un Campeonato Mundial.', cond: e => e.mundiales >= 1 },
  { id: 'dominio', emoji: '👑', nombre: 'Dinastía', desc: () => 'Tres o más títulos: no fue suerte.', pista: 'Acumula tres títulos en una misma carrera.', cond: e => e.titulos.length >= 3 },
  { id: 'ochomedallas', emoji: '🎖️', nombre: 'Las ocho medallas', desc: e => `Completaste el circuito de ${e.regionNombre}.`, pista: 'Consigue las ocho medallas de una región.', cond: e => e.medallas >= 8 },
  { id: 'coleccionista', emoji: '📦', nombre: 'Coleccionista', desc: e => `${e.capturasTotales} Pokémon distintos pasaron por tu equipo.`, pista: 'Que pasen doce Pokémon distintos por tu equipo.', cond: e => e.capturasTotales >= 12 },
  { id: 'legendario', emoji: '⚡', nombre: 'Domador de leyendas', desc: () => 'Capturaste un Pokémon legendario. Casi nadie puede decirlo.', pista: 'Captura un legendario. Casi nadie puede.', cond: e => e.equipo.some(p => p.rareza === 'legendario') },
  { id: 'pseudo', emoji: '🐲', nombre: 'Criador de dragones', desc: () => 'Criaste un pseudolegendario desde su primera etapa.', pista: 'Cría un pseudolegendario desde su primera etapa.', cond: e => e.equipo.some(p => p.rareza === 'pseudo' && p.etapa >= 2) },
  { id: 'fiel', emoji: '💚', nombre: 'Hasta el final', desc: () => 'Tu primer compañero siguió contigo toda la carrera.', pista: 'Retírate con tu primer compañero todavía en activo.', cond: e => e.equipo.some(p => p.uid === e.socio && !p.retirado) },
  { id: 'vinculo', emoji: '🫂', nombre: 'Alma gemela', desc: () => 'Vínculo máximo con tu equipo. Se lanzarían al fuego por ti.', pista: 'Llega a 90 de vínculo con tu equipo.', cond: e => e.stats.vinculo >= 90 },
  { id: 'idolo', emoji: '📣', nombre: 'Ídolo de masas', desc: () => 'Fama por encima de 85. Te paran por la calle.', pista: 'Llega a 85 de fama.', cond: e => e.stats.fama >= 85 },
  { id: 'millonario', emoji: '💰', nombre: 'Millonario', desc: e => `Te retiraste con ${Math.round(e.dinero / 1000)}k ₽ en el banco.`, pista: 'Retírate con un millón en el banco.', cond: e => e.dinero >= 1000000 },
  { id: 'hierro', emoji: '🦾', nombre: 'De hierro', desc: () => 'Ni una sola lesión grave en toda la carrera. Otra pasta.', pista: 'Acaba la carrera sin lesiones crónicas y con salud.', cond: e => !e.flags.lesionCronica && e.stats.salud >= 55 },
  { id: 'eterno', emoji: '⏳', nombre: 'Eterno', desc: e => `Competiste hasta los ${e.edad}.`, pista: 'Sigue compitiendo a los 32.', cond: e => e.edad >= 32 },
  { id: 'trotamundos', emoji: '✈️', nombre: 'Trotamundos', desc: e => `Compitiste en ${e.regionesVisitadas.length} regiones distintas.`, pista: 'Compite en tres regiones distintas.', cond: e => e.regionesVisitadas.length >= 3 },
  { id: 'lider', emoji: '🏛️', nombre: 'Líder de Gimnasio', desc: () => 'Te dieron un gimnasio propio y lo aceptaste.', pista: 'Acepta un gimnasio propio.', cond: e => !!e.flags.liderGimnasio },
  { id: 'altomando', emoji: '🛡️', nombre: 'Alto Mando', desc: () => 'Ocupaste una silla del Alto Mando.', pista: 'Ocupa una silla del Alto Mando.', cond: e => !!e.flags.altoMando },
  { id: 'heroe', emoji: '🦸', nombre: 'Héroe', desc: () => 'Plantaste cara al crimen organizado cuando nadie más lo hizo.', pista: 'Planta cara al crimen organizado.', cond: e => !!e.flags.heroe },
  { id: 'villano', emoji: '🕳️', nombre: 'Zonas grises', desc: () => 'Cobraste por mirar hacia otro lado. Tú sabrás.', pista: 'Cobra por mirar hacia otro lado.', cond: e => !!e.flags.traicion },
  { id: 'sancionado', emoji: '⛔', nombre: 'El sancionado', desc: () => 'La federación te apartó del circuito. Volviste igual.', pista: 'Sé sancionado por la federación y vuelve.', cond: e => !!e.flags.exsancionado },
  { id: 'nemesis', emoji: '⚔️', nombre: 'El duelo eterno', desc: e => `Le ganaste la carrera a ${e.rival.nombre} por ${e.rival.derrotasTuyas}-${e.rival.victoriasSuyas}.`, pista: 'Gánale la carrera a tu rival por tres victorias o más.', cond: e => e.rival.derrotasTuyas > e.rival.victoriasSuyas && e.rival.derrotasTuyas >= 3 },
  { id: 'maquina', emoji: '💥', nombre: 'La máquina', desc: e => `${e.victorias} victorias en la carrera.`, pista: 'Suma 300 victorias en tu carrera.', cond: e => e.victorias >= 300 },
  { id: 'invicto', emoji: '📈', nombre: 'Récord impecable', desc: e => `${e.victorias}-${e.derrotas}: ganaste el triple de lo que perdiste.`, pista: 'Gana el triple de combates de los que pierdes.', cond: e => e.victorias > e.derrotas * 3 },
  { id: 'techo', emoji: '🚀', nombre: 'Los noventa', desc: e => `Alcanzaste una media de ${Math.round(e.media)}. Ahí arriba no hay casi nadie.`, pista: 'Alcanza una media de 90.', cond: e => e.media >= 90 },
  { id: 'maestroobj', emoji: '🎒', nombre: 'Mochila legendaria', desc: e => `Reuniste ${e.objetos.length} objetos de entrenamiento.`, pista: 'Reúne cinco objetos de entrenamiento.', cond: e => e.objetos.length >= 5 },
  { id: 'santuario', emoji: '🏞️', nombre: 'El santuario', desc: () => 'Fundaste un refugio para Pokémon retirados.', pista: 'Funda un refugio para Pokémon retirados.', cond: e => !!e.flags.santuario },
  { id: 'maestro', emoji: '🌱', nombre: 'Maestro de una generación', desc: () => 'Formaste al que vino después de ti.', pista: 'Forma al entrenador que viene detrás de ti.', cond: e => !!e.flags.discipulo },
  { id: 'retirodigno', emoji: '🎬', nombre: 'Se fue por la puerta grande', desc: () => 'Elegiste tú cuándo parar, sin que nadie te empujara.', pista: 'Retírate cuando lo decidas tú.', cond: e => e.causaRetiro === 'eleccion' },
];
